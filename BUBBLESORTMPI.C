...
int main(int argc, char** argv) {
 ...
 MPI_Scatter(data, local_size, MPI_INT, local_data, local_size, MPI_INT, 0, 
MPI_COMM_WORLD);
 // Perform local Quick Sort
 quicksort(local_data, 0, local_size - 1);
 // Gather sorted subarrays from all processes
 MPI_Gather(local_data, local_size, MPI_INT, data, local_size, MPI_INT, 0, 
MPI_COMM_WORLD);
 ...
 int local_size = n / size;
 int* local_data = (int*)malloc(local_size * sizeof(int));
 MPI_Scatter(NULL, local_size, MPI_INT, local_data, local_size, MPI_INT, 0, 
MPI_COMM_WORLD);
 quicksort(local_data, 0, local_size - 1);
